...
class SparkFetcherSuite extends AnyFunSuite with MockitoSugar {

  test("SparkFetcher should fetch and process application data") {
    ...
    var sparkFetcher = SparkFetcher
    sparkFetcher.sparkRestClient = restClient
    val fetchedData = sparkFetcher.fetchData("http://localhost:18080/history/app-id/1")

    assert(fetchedData.appId === "app-id")
  }
}
