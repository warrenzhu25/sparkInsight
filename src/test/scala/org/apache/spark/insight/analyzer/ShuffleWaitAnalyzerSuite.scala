...
class ShuffleWaitAnalyzerSuite extends AnyFunSuite {

  test("ShuffleWaitAnalyzer should identify stages with high shuffle wait") {
    ...
    val stageData = Seq(
      new StageData(
        ...
        shuffleFetchWaitTime = TimeUnit.MINUTES.toMillis(3),
      ...
      new StageData(
        ...
        shuffleRemoteBlocksFetched = 0,
        shuffleLocalBlocksFetched = 0,
        shuffleFetchWaitTime = TimeUnit.MINUTES.toMillis(1),
        shuffleRemoteBytesRead = 0,
        shuffleRemoteBytesReadToDisk = 0,
        shuffleLocalBytesRead = 0,
        shuffleReadBytes = 0,
    ...
    val result = ShuffleWaitAnalyzer.analysis(appData)

    assert(result.rows.size === 1)
    assert(result.rows.head(0) === "1.0")
    assert(result.rows.head(1) === "stage1")
    assert(result.rows.head(2) === "3")
  }
}
